...
class QuizSystem {
  ...
  checkFillBlank() {
    ...
    if (!input || !feedback) {
      console.error('Missing quiz elements for fill blank check');
      return;
    }
  ...
  checkLetterPicker() {
    ...
    const letterAnswerEl = document.querySelector('.letter-picker-answer');
    const feedback = document.querySelector('.quiz-feedback');
    const checkButton = document.querySelector('.quiz-check');
    
    if (!textInput || !letterAnswerEl || !feedback) {
      console.error('Missing quiz elements for letter picker check');
      return;
    }
  ...
  startEndlessQuiz(containerId) {
    ...
    // Create the endless quiz container
    container.innerHTML = '<div class="endless-quiz-container"></div>';
    const quizContainer = container.querySelector('.endless-quiz-container');
    
    if (!quizContainer) {
      console.error('Failed to create quiz container');
      return;
    }

    // Get the topic from the container ID
    const topicIndex = containerId.replace('quiz', '');
    const topics = ['seasons', 'vocabulary', 'expressions', 'dialogue', 'extraVocabulary', 'grammar'];
    const topic = topics[parseInt(topicIndex)] || 'vocabulary';
    
    // Generate and render the first quiz
    const quiz = this.generateQuiz(topic);
    if (quiz) {
      this.renderQuiz(quiz, containerId);
    } else {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Also handle direct onclick attributes
  document.querySelectorAll('[onclick*="toggleQuiz"]').forEach(element => {
...
function toggleQuiz(id) {
  ...
  if (!block) {
    console.error('Quiz block not found:', id);
    return;
  }
  ...
  if (isHidden) {
    ...
    // Initialize the endless quiz properly
    quizSystem.startEndlessQuiz(id);
  }
}
